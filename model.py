...
from data_utils import minibatches, pad_sequences, get_chunks, label2ind_ret, max_words_in_sent
    ...
    def add_word_embeddings_op(self):
        ...
        with tf.device("/cpu:0"),tf.variable_scope("pref"):   ########### This is the original one
            _pref_embeddings = tf.get_variable(name="_pref_embeddings", dtype=tf.float32,
                                               trainable=self.config.train_embeddings,shape=[9000, self.config.dim_pref])  ## max number of words in the sentence
        ...
        with tf.device("/cpu:0"),tf.variable_scope("suff"):
            _suff_embeddings = tf.get_variable(name="_suff_embeddings", dtype=tf.float32,
                                               trainable=self.config.train_embeddings,shape=[9000, self.config.dim_suff])  ## max number of words in the sentence
